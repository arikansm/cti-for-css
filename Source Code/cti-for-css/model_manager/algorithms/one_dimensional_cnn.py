...
class OneDimensionalCNN(Algorithm):
    ...
    def train(self, train_functions: list[str], train_labels: list[str],
        ...
        model.add(layers.Conv1D(256, 7, padding="valid", activation="relu", strides=3))
        model.add(layers.Conv1D(256, 7, padding="valid", activation="relu", strides=3))
        ...
        self.__logger.debug("one-dimensional convolutional neural network has been started to learn")
        history = None
        ...
        self.__logger.debug("one-dimensional convolutional neural network model is acquired")

        model.save(f"{ConfigurationManager.DeepLearning.Method.model_save_path}/one_d_cnn.h5", save_format="h5")
    ...
    def evaluate(self, test_functions: list[str], test_labels: list[str],
        ...
        self.__logger.debug("one-dimensional convolutional neural network model has been started to evaluation")
        test_dataset = Algorithm._prepare_test_dataset(serialized_vectorizer, test_functions, test_labels,
                                                       self.maximum_size_of_vocabulary, self.sequence_length)
        model.evaluate(test_dataset)
        self.__logger.debug("one-dimensional convolutional neural network model is evaluated")
